...
const Home: NextPage = () => {
    ...
    const whatRef = useRef<HTMLElement | null>(null);
    ...
    function handleChangeSection(linkIndex: number, linkName: string) {
        ...
        if (linkName === 'what?') {
            ...
            const sectionEl = document.getElementById(linkName);
    ...
    // auto-set current link using intersection observer, nothing fancy just toggle basically.
    const entry = useIntersectionObserver(whyRef, {});
    const isVisible = !!entry?.isIntersecting;

    useEffect(() => {
        if (!whyRef.current) return;

        const sectionID = whyRef.current.id;
        if (isVisible) {
            setCurrentLink({ id: 1, name: sectionID });
        } else {
            setCurrentLink({ id: 0, name: 'what?' });
        }
    }, [isVisible, setCurrentLink]);
    ...
    return (
        <HomeLayout>
            <div className="relative flex flex-col lg:flex-row justify-center gap-x-10 items-center h-auto w-full px-10 lg:px-0 2xl:max-w-[1600px]">
                {/* Page scroll menu */}
                <div className="lg:fixed lg:left-[5%] xl:left-[10%] top-10 flex lg:flex-col items-center lg:h-screen w-full lg:w-auto border-b-1 dark:border-slate-600 lg:border-0 md:shadow-none self-center justify-center py-10 lg:py-0 lg:mx-0">
                    {/* desktop styling */}
                    <nav
                        className="hidden lg:block border-l-2 transition-color duration-500"
                    ...
                    <nav className="lg:hidden flex w-full items-center h-auto">
                ...
                <div className="flex flex-col justify-center items-center h-auto right-auto w-full lg:max-w-[70%] xl:max-w-80%] lg:px-20 gap-y-[100px] lg:gap-y-40 lg:py-0 py-20">
                    <section
                        ref={whatRef}
                        id="what?"
                        className="flex flex-col h-auto lg:min-h-screen items-start justify-start lg:justify-center"
                    >
                        ...
                        <div className="flex flex-col mt-5">
                            <p className="text-slate-800 dark:text-slate-400">
                                ...
                                externally, solely relying on data stored within
                                your own browser to operate. You can manually
                                delete Kan-banana&#39;s database by opening up
                                developer tools (right click & inspect) and
                                following:
                            </p>
                            <br />
                            <br />
                    ...
                    >
                        ...
                        <p className="text-slate-700 dark:text-slate-400">
                            ...
                            solely with the aim of learning react/next.js. It is
                            the first actual application I created and released
                            and, as I finished the development of it quite some
                            time ago, there are many things I would do
                            differently now. As it is a free app, I have also
                            not spent too much time in testing and optimising.
                        </p>
                        <br />
                        <p className="text-slate-700 dark:text-slate-400">
                            The Specific skills that I wished to cover with the
                            creation of Kanbanana were as follows...
                        </p>
                        <ul className="grid grid-cols-1 md:grid-cols-2 my-10 gap-10">
                        ...
                        <p className="text-slate-700 dark:text-slate-400">
                            If you wish to see more of my work or get in contact
                            with me, please check out{' '}
                ...
                {/* side slider wrapper */}
                <div className="hidden fixed lg:right-[5%] xl:right-[10%] lg:flex self-start lg:h-screen mt-[10vh]">
                    <span
                        className={clsx(
                            'flex w-[3px] h-1/4 transition-translate duration-500 rounded-full',
                            currentLink.name === 'why?' && 'translate-y-[50vh]'
                        )}
